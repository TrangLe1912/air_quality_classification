...
def clean_air_quality_df(df: pd.DataFrame) -> pd.DataFrame:
    """
    Missing indicated as NA. We'll coerce numeric columns and parse datetime.
    """
    df = df.copy()
    ...
    # Ensure station / wd as string
    for col in ["station", "wd"]:
        if col in df.columns:
            df[col] = df[col].astype("string")

    # Sort (important for rolling)
...
def pm25_to_aqi_class(pm25_ug_m3: pd.Series) -> pd.Series:
    """
    Updated PM2.5 breakpoints (µg/m³):
      Good:     0.0–9.0
      Moderate: 9.1–35.4
      USG:      35.5–55.4
...
def add_pm25_24h_and_label(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["aqi_class"] = pm25_to_aqi_class(df["pm25_24h"])
    return df
...
def train_classifier(
    ...
    y_train = train_df[target_col].astype("string")

    X_test = test_df[feature_cols]
    y_test = test_df[target_col].astype("string")

    cat_cols = [c for c in feature_cols if train_df[c].dtype.name in ("string", "object")]
    ...
    metrics = {
        ...
        "accuracy": float(accuracy_score(y_test[test_mask], y_pred)),
        "f1_macro": float(f1_score(y_test[test_mask], y_pred, average="macro")),
        "report": classification_report(y_test[test_mask], y_pred, output_dict=True),
        "confusion_matrix": confusion_matrix(y_test[test_mask], y_pred, labels=AQI_CLASSES).tolist(),
        "labels": AQI_CLASSES,
    ...
    pred_df = pd.DataFrame({
        ...
        "y_true": y_test[test_mask].values,
        "y_pred": y_pred,
    })
...
# -----------------------------
def run_prepare(paths: Paths, use_ucimlrepo: bool = True, raw_zip_path: str | None = None) -> Path:
